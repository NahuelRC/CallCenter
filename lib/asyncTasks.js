...
// Construye un "to" válido para Twilio o null si no es posible
function safeToWa(fromWa, e164) {
  // Si Twilio ya envió 'whatsapp:+...' lo usamos
  if (fromWa && /^whatsapp:\+\d{7,15}$/.test(fromWa)) return fromWa;
  // Si tenemos E.164 (+...) construimos el prefijo whatsapp:
  if (e164 && /^\+\d{7,15}$/.test(e164)) return `whatsapp:${e164}`;
  return null;
}
...
export async function handleInboundAsync(reqBody) {
  ...
  // Fallback: si sigue sin imagen, intentamos con cualquier producto con imagen (activo; si no hay, sin filtro)
  if (ai.wantImage && mediaOut.length === 0) {
    try {
      ...
      if (any?.images?.[0]?.url) {
        mediaOut.push(any.images[0].url);
        console.log("[IMG FALLBACK] using first available product image:", any.images[0].url);
      } else {
        console.log("[IMG FALLBACK] no product with images found.");
      }
    } catch (e) {
  ...
  try {
    ...
    const mediaOutClean = (Array.isArray(mediaOut) ? mediaOut : [])
      ...
      // 4.2 Whitelist dominio (Cloudinary)
      .filter(u => MEDIA_DOMAIN_WHITELIST.some(rx => rx.test(u)))
      // 4.3 Debe parecer imagen directa
      .filter(looksLikeImageUrl);

    // DEBUG: ver exactamente qué URLs van a salir
    console.log("[MEDIA OUT]", mediaOutClean);

    // Normalizar 'to': si falta From en webhook, reconstruimos con E.164 normalizado
    const toWa = safeToWa(fromWa, from);
    ...
    if (!toWa) {
      console.warn("[TW SEND] skip: invalid 'to' (fromWa:", fromWa, ", e164:", from, ")");
      sid = null;
    } else if (mediaOutClean.length === 0 && text.length === 0) {
