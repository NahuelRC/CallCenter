...
const IMG_KEYWORDS = [
  ...
  "catálogo", "catalogo",
  "gota", "gotas", "gotitas", "drop", "drops" // ← agregado
];
...
function extractQueryFromBody(text = "") {
  ...
  if (m && m[1]) {
    // limpia artículos iniciales del fragmento capturado (ej: "las gotas" -> "gotas")
    const frag = m[1].trim().replace(/^(la|el|los|las)\s+/i, "").trim();
    return frag || null;
  }
...
export async function handleInboundAsync(reqBody) {
  ...
  // Fallback: si sigue sin imagen, intentamos con cualquier producto con imagen (activo)
  if (ai.wantImage && mediaOut.length === 0) {
    try {
      ...
      // Si no hay activos con imagen, probá sin el filtro active:true
      if (!any) {
        any = await Product.findOne({ "images.0.url": { $exists: true, $ne: "" } })
          .select({ images: { $slice: 1 } })
          .lean();
      }
  ...
  try {
    ...
    const toWa = fromWa || `whatsapp:${from}`;
