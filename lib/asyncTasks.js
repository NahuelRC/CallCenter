...
export async function handleInboundAsync(reqBody) {
  ...
  function createDefaultAIPlan() {
    return { text: "👍", wantImage: false, sku: null, productName: null, imageHint: null };
  }
  let aiPlan = createDefaultAIPlan();
  try {
    const promptInput =
    ...
    aiPlan = createDefaultAIPlan();
    aiPlan.text = "Gracias por tu mensaje. En breve te respondemos 🙌";
  }
  ...
  try {
    ...
    if (mediaOut.length) {
      // Validar que cada elemento sea una URL válida
      const validUrls = mediaOut.filter(url => {
        try {
          new URL(url);
          return true;
        } catch {
      ...
      if (validUrls.length) payload.mediaUrl = validUrls; // adjuntamos imágenes si hay
    }
